...
const Cardtemplate = (props) => {
  ...
  return (
    <NavLink link to={`/${mediatype}/${id}`}>
      <div className='flex flex-col h-[270px] md:h-[470px] gap-[14px] md:gap-8 overflow-hidden relative  '>
          <div className=' min-h-[200px] min-w-[135px] md:min-h-[350px] md:min-w-[237px] overflow-hidden rounded-md hover:cursor-pointer relative '>
              {/* <Img src={props.path}/> */}
              <img className=' h-[200px]  md:h-[350px] w-full  md:img ' src={props.path} alt="" />
              { props.genres && (<div className='genre'><Genres data={props.genres}/></div>)}
          </div>
          <div className=' absolute w-[30px] md:w-[50px] bottom-[58px] left-3 md:left-3 md:bottom-[96px]'><CircularProgressbar  maxValue={10}   value={props.rating} text={newrate} className=' p-[2px] rounded-[50%] font-[700] bg-white'
          ...
          <div className='text-white flex flex-col gap-1 md:gap-2'>
            <h4 className='text-[16px] overflow-hidden md:text-[21px]'>{naming.slice(0,20)}</h4>
